...
export const EnterCommits: Story = {
  play: async ({ canvas, args }) => {
    await userEvent.click(canvas.getByRole('button', { name: /Rename orders-api/ }))
    const field = await canvas.findByLabelText('Name')
...
/**
 * Clicking away saves: the inline-rename convention everywhere it appears, so
...
/**
 * Escape reverts, and does not save on its way out. Leaving the field is what
 * commits and cancelling leaves the field, so the two would collide: the
...
export const ServerRefuses: Story = {
  args: {
    onRename: fn(async () => {
      throw new Error('An instance named "checkout-api" already exists')
    }),
  },
  play: async ({ canvas }) => {
    await userEvent.click(canvas.getByRole('button', { name: /Rename orders-api/ }))
    ...
    await expect(canvas.getByLabelText('Name')).toHaveValue('checkout-api')
  },
}
