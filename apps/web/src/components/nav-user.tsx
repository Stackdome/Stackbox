...
export function NavUser({
  ...
  user: {
    ...
    email: string
    avatar: string
    /** Shown under the name. The organisation is the switchable context, and
     *  this menu is where switching lives; an email is not switchable, so it
     *  does not earn a permanent line. */
    organisation?: string
  }
}) {
      ...
      <SidebarMenuItem>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            {/* 40px, so the two lines and the 28px avatar sit without
                crowding: the one row in the frame that is not on the 32px
            ...
            <SidebarMenuButton
              ...
              // no padding (`size-8!` + the `lg` size's `p-0!`), which is right
            ...
            >
              ...
              {/* `rail-x` is a track that closes to zero: it clips exactly ONE
                  child, so both of these carry a wrapper. */}
