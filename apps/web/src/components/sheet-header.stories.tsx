...
/** Declares the page a journey, the way the New application page will. */
...
export const Nested: Story = {
  render: () => <Harness path="/instances/acme-web/detail" />,
}

/** A journey: back arrow, divider, and the title alone. No trail. */
export const Journey: Story = {
  render: () => <Harness path="/applications/new" journey />,
}
...
export const JourneyDropsTheTrail: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    // "New" is the title; "Applications" would be the crumb we deliberately dropped.
    await expect(canvas.queryByRole('link', { name: 'Applications' })).not.toBeInTheDocument()
  },
}
...
export const NestedHasNoBackArrow: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByRole('link', { name: 'Instances' })).toBeInTheDocument()
  },
}

/** Arrived by navigating: back steps the way you came, not to the origin. */
export const BackStepsThroughHistory: Story = {
  render: () => <Harness path="/applications/new" from="/repositories" journey />,
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement)
    await expect(canvas.getByTestId('where')).toHaveTextContent('/applications/new')
    await userEvent.click(canvas.getByRole('button', { name: 'Back' }))
    await expect(canvas.getByTestId('where')).toHaveTextContent('/repositories')
  },
}
