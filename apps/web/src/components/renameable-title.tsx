 ...
 *
 * Enter commits, clicking away commits, Escape reverts: the inline-rename
 * convention everywhere it appears, so a title that behaved differently would
 * be the one control on the screen you had to learn.
 *
 ...
 * rather than starting again: the constraint arrives next to the control that
 * broke it.
 */
export function RenameableTitle({
  ...
  // The name can change under us: another tab, a refetch. An edit in flight
    ...
    return (
      <button
        ...
        className={cn(
          ...
          // The rung comes from `PageTitle`, not from here: the same slot in
          // its other state must draw at the same size.
          pageTitleClass,
          "transition-colors",
  ...
  return (
    <span className="relative flex items-center">
      <input
        ...
        // Leaving the field is what saves. No confirm buttons: the header is
        // not a form, and the two exits people already know are Enter and
        // Escape.
        onBlur={() => void commit()}
        onKeyDown={(e) => {
          if (e.key === "Enter") {
