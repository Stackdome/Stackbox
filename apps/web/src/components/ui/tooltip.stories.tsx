...
export const Default: Story = {
    ...
    <div className="p-16">
      <Tooltip>
        <TooltipTrigger asChild>
          <Button variant="outline" size="icon" aria-label="About this instance">
            <Info />
          </Button>
        </TooltipTrigger>
  ...
  play: async ({ canvas }) => {
    await userEvent.hover(canvas.getByRole('button', { name: 'About this instance' }))
    await waitFor(async () => {
...
export const LongText: Story = {
  render: () => (
    <div className="p-20">
      <Tooltip open>
        <TooltipTrigger asChild>
          <Button variant="outline" size="icon" aria-label="What is an instance?">
            <Info />
          </Button>
        </TooltipTrigger>
        <TooltipContent className="max-w-64">
          An instance is your app and everything it needs to run: services,
