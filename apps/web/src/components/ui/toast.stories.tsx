...
/**
 * **Only the OPEN ones.** A dismissed toast keeps `data-swipe-direction` for
 * the 250ms its exit animation runs, so a plain query sees the last story's
 * toasts on their way out as well as this story's.
 */
const open = () => Array.from(document.querySelectorAll<HTMLElement>('[data-swipe-direction][data-state="open"]'))
 ...
 * carried by the glyph alone: a tinted border per tone would say the severity
 * twice and make four components that look like four different things.
 */
export const EveryTone: Story = {
  args: {
    toasts: [
      { title: 'orders-api released to checkout-web.', variant: 'success' },
      { title: 'Could not reach the instance', description: 'The deploy was not started.', variant: 'destructive' },
...
/**
 * **Bottom right, 16 clear of both edges.** The corner has no chrome to be
 * mistaken for.
 */
...
export const TitleAndDescriptionAreOneSentence: Story = {
  ...
  play: async () => {
    ...
    await expect(
      await view().findByText(/Service created\. Provisioning has started/),
    ).toBeInTheDocument()

    const title = await view().findByText('Service created')
...
export const TheClockScalesWithTheWords: Story = {
  ...
  play: async () => {
    ...
    await expect(
      readingDuration('Could not reach the instance', 'The deploy was not started.'),
    ).toBeGreaterThan(4000)
    await expect(
      readingDuration(
        'Registry removed',
        'Instances affected: web-storefront, checkout-api, billing-worker. Their image pulls or pushes may fail until another credential covers docker.io, so check each one before the next deploy.',
      ),
    ).toBe(10000)
