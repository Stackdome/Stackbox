...
const field = (label: string, value: string, id: string, hint?: string) => (
  <div key={id} className="flex flex-col gap-1.5">
    ...
    {hint && <p className="text-fg-2 text-meta">{hint}</p>}
  </div>
)
  ...
  render: () => (
    <Drawer defaultOpen>
      ...
      <DrawerContent size="form">
        <DrawerHeader
          title="New secret"
          description="Available to every service in this instance, at build and run time."
        />
        <DrawerBody>
      ...
      <DrawerContent size="work">
        ...
        <DrawerFooter>
          <Alert variant="destructive">
            <AlertDescription>
              A store named backups-eu already exists: pick another name, or
 ...
 *
 * The path is the way back, so the phrase starts on the header's own 20
 * column, the same x as the body beneath it.
 */
export const TheHeaderOwnsTheBand: Story = {
  render: () => (
    <Drawer defaultOpen>
      <DrawerContent size="work">
        <DrawerHeader steps={['New instance', 'Select a service']} />
  ...
  play: async () => {
    ...
    // **The path starts on the 20 column.**
...
export const CrumbsGoBack: Story = {
      ...
      <Drawer defaultOpen>
        <DrawerContent size="work">
          <DrawerHeader
            steps={
              step === 1
                ? ['New instance', 'Select a service']
                : [{ label: 'New instance', onClick: () => setStep(1) }, 'A repository']
            }
          />
          <DrawerBody>{field('Name', 'acme-web', 'n')}</DrawerBody>
        </DrawerContent>
  ...
  play: async () => {
    ...
    const crumb = within(path()).getByRole('button', { name: 'New instance' })
    await expect(within(path()).queryByRole('button', { name: 'A repository' })).toBeNull()

    await userEvent.click(crumb)

    // Step one: `Select a service` is now the title, and `New instance` went
    // dead: it points at the screen you are already on.
    await expect(await within(path()).findByText('Select a service')).toBeVisible()
    await expect(within(path()).queryByRole('button', { name: 'New instance' })).toBeNull()
  },
}
