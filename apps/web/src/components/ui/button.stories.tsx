...
const meta = {
  ...
  component: Button,
  tags: ['ai-generated'],
  args: { children: 'Spin up instance' },
          ...
          <div className="flex flex-wrap items-center gap-3">
            {(['default', 'secondary', 'outline', 'ghost', 'destructive', 'destructive-ghost'] as const).map((variant) => (
              <Button key={variant} variant={variant} shape={shape}>
                {shape === 'pill' ? 'Spin up instance' : 'Save'}
              </Button>
            ))}
          </div>
        </div>
...
export const Loading: Story = {
  args: { loading: true, loadingText: 'Spinning up…', children: 'Spin up instance' },
  play: async ({ canvas }) => {
    const btn = canvas.getByRole('button')
    await expect(btn).toBeDisabled()
    await expect(btn).toHaveAttribute('aria-busy', 'true')
    await expect(btn).toHaveTextContent('Spinning up…')
  },
}
...
export const Disabled: Story = {
  args: { disabled: true },
  play: async ({ canvas }) => {
    await expect(canvas.getByRole('button', { name: /spin up instance/i })).toBeDisabled()
  },
}
...
export const LoadingIsNotDisabled: Story = {
  args: { loading: true, loadingText: 'Spinning up…' },
  play: async ({ canvas }) => {
    const btn = canvas.getByRole('button')
    const style = getComputedStyle(btn)
    await expect(btn).toBeDisabled()
...
export const KeyboardFocusOutline: Story = {
  play: async ({ canvas, userEvent }) => {
    const button = canvas.getByRole('button', { name: /spin up instance/i })
    await userEvent.tab()
