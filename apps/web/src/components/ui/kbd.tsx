 ...
 * sheet, with no variant per ground.
 *
 * ### 24 inside 32, four from the edge
 *
 * **No face, no border.** A filled box inside a filled button is a button
 * within a button, and across a screenful of them a second filled rectangle
 * per button reads as clutter. The glyphs alone say *key*: they are ⌘ and
 * `corner-down-left`, marks that exist nowhere else in the product.
 *
 * With no face the cap is simply CONTENT, so it lives inside the button's own
 * padding: 12 from the right edge, the same inset the label has on the left.
 *
 * | | |
 * |---|---|
  ...
  return (
    <kbd
      ...
      className={cn(
        ...
        // Dimming the element's own `color` would drag a derived background
        // tint down with it, so opacity is applied on the children instead.
