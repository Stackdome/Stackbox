 ...
 *
 * It is **not** the `Table` primitive. `Table` is right for dense tabular
 * data, such as `backups-list.tsx` or the users list, where rows are ~40px, a
 * rule between them earns its place, and `text-body` is the size. A list PAGE
 * is a different object: 64px rows, no rule, a 14px name over a 12px second
 * line, and a 20px column gap. Every one of those is a `Table` default
 * overridden at the call site, and five pages overriding the same seven
 * defaults is a fork wearing a primitive's name.
 *
...
export function DataListHeader({
  ...
  columns: string;
  /** One per track. An empty string draws the track and no label, which is
   *  how an actions or chevron column is titled, never with the word "Actions". */
  labels: string[];
}) {
  return (
    <div
 ...
 * §6: mono means a machine produced this and a machine will read it back.
 * Five of the seven callers pass a pure machine value (a host, a cluster id,
 * a branch), so `true` stays the default. But a line like
 * `default · main@a3f9d2e` is a project NAME and a ref, and setting the whole
 * thing in mono makes the project look like something you could paste into a
 * terminal.
 *
 ...
 *
 * **Data is `foreground`, chrome is `fg-muted`.** The header stays muted
 * because a header is chrome; what it labels is the thing you came to read.
 *
 * A call site may still drop a cell to `fg-2` where it is genuinely
 * supporting rather than primary; previews does it for the repository name,
 * which qualifies the environment beside it.
 */
