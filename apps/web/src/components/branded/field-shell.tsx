...
interface FieldShellProps {
   ...
   *
   * **Default 2: a field fills the body unless it has a partner.**
   *
...
export function FieldShell({
  ...
  const labelNode = (
    // 2, so the SEEN gap is 6: the 20px target carries 4px of air around its
    // 12px mark, so the row pays 2 and the box pays 4.
    //
  ...
  // `text-wrap: balance` is set globally (`p { text-wrap: balance }` in
  // index.css), so this hint and every other paragraph evens its lines
  // rather than dumping the remainder on the last one. Nothing to set here.
  //
  ...
  // edges on the grid.
  //
  // **It reaches every descendant on purpose**, not just a direct child:
  // nested controls in a column are the common case, and a control that
  // needs its own width says so with `!`.
  const fill = "[&_[data-slot=select-trigger]]:w-full";

  // **The control is centred on the whole statement, and stands 24 off it.**
  // It answers the label AND its hint together, so it centres on both.
  //
  // 24, not 16: at 16 the switch reads as the last word of the sentence rather
  ...
  //
  // `//` in child JSX position is not a comment; it is rendered as text.
