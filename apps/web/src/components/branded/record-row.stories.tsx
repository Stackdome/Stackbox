...
const meta = {
  ...
  component: RecordRow,
  tags: ['ai-generated'],
  // 480 is the inspector's width, the constraint the sizing rule targets; at
  // any wider size a record fits however you lay it out.
  decorators: [
    (Story) => (
...
export const OneRecord: Story = {
  ...
  play: async ({ canvasElement }) => {
    // The remove button carries the row's identity rather than a bare X;
    // most of a record's actions are otherwise announced as nothing.
...
export const LabelledByItsOwnName: Story = {
  args: { children: <PortMembers /> },
  render: () => (
    <FormSection label="Variables" state="2 variables">
      <RecordList>
...
/**
 * A repository row: name, branch, and its status in its own track to the
 * right, never crowding the name it describes.
 */
export const StatusInItsOwnTrack: Story = {
  args: {
    children: (
      <div className="flex h-16 w-full items-center justify-between">
        <div className="flex flex-col justify-center gap-0.5">
          <span className="text-body font-medium text-foreground">checkout-web</span>
          <span className="text-meta text-fg-muted">main</span>
        </div>
        <StatusText domain="generic" state="Running" />
      </div>
    ),
  },
  play: async ({ canvas, canvasElement }) => {
    // The meta decorator wraps every story in a bordered, padded frame, so the
    // row itself is the decorator's first child, not the canvas's.
    const row = canvasElement.firstElementChild!.firstElementChild as HTMLElement
    const name = canvas.getByText('checkout-web')
    const status = canvas.getByText('Running')
