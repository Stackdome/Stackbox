...
interface KeyValueRowsProps<T extends KeyValueRow> {
  ...
  removeLabel?: string;
  /**
   * The empty state's first line, for example `No variables` or `No keys`.
   *
 ...
 *
 * **The add control is a row, not a button beside the label.** A `+ Add`
 * tucked up next to the group's heading reads as chrome and sits nowhere near
 * the last row it extends; full width and directly under the list, it reads
 ...
 *
 * **The placeholders are lower case.** A placeholder in caps is not the field
 * shouting a convention, it is a placeholder pretending to be a value. Env
 * vars are conventionally upper case and secret keys are not, so caps would
...
export function KeyValueRows<T extends KeyValueRow>({
  ...
  const addButton = (full: boolean) => (
    ...
    // on purpose, for sitting beside an outline button; alone against a
    // column of fields it just reads as another one.
    <Button
      type="button"
      variant="outline"
      shape="flat"
          ...
          <div key={index} className="flex flex-col gap-1">
            <div className="flex items-center gap-2">
              <Input
                ...
                // control, in which case the key becomes a fixed width so the
                // slack goes to the value, which is usually longer.
                //
