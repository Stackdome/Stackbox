  ...
  return (
    <div
      className={cn(
        ...
        // real gap between them, which is the whole fix (see the note below), and
        // it is the shape `CommandInput` already uses for every other search
        // inside a menu.
    ...
    >
      {/* 16px glyph on the field's own inset, with an 8px gap to the text: the
          ...
          **`bare`: the glyph shares the row's LEFT EDGE, not its centre.**
          ...
          Two reasons, both visible the moment it is on screen. A chip is a
          32px face and the search glyph is a bare 16px mark, so aligning their
          *centres* aligns two things the eye does not read as the same kind of
          object; and holding that centre forces a **20px** gap to the
          placeholder, which unbinds the glyph from the words it belongs to. It
          stopped reading as a search box and started reading as an icon that
          happened to be near some text.
