...
`tokens.css` in the bundle is typically a copy of `apps/web/src/index.css` at design time. Before treating any token as new:

- Diff bundle `tokens.css` against the live `apps/web/src/index.css`.
...
2. **Token mapping** — a table mapping bundle `tokens.css` ↔ `apps/web/src/index.css` (token name, bundle value, live token, match/drift/new).
...
- Inventing new colors because `tokens.css` "looks different" → diff against `apps/web/src/index.css`; reuse existing tokens.
