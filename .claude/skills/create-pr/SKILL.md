...
description: >-
  ...
  for code review. Runs a hard pre-flight gate (`pnpm -r lint && pnpm -r
  test:run && pnpm -r typecheck`) and refuses to push if anything fails. Do NOT use
...
- **`apps/web/` scope:** any path under `apps/web/`.
- **`apps/api/` scope:** any path under `apps/api/`.
- **Docs/assets only:** changes confined to `docs/`, `*.md`, `.claude/`, screenshots, and similar non-build paths. Skip the check suite — there's nothing to verify.

### Step 0.2: Run checks (only if apps/web or apps/api scope)

```bash
pnpm -r lint && pnpm -r test:run && pnpm -r typecheck
```

Pass criteria:
- `pnpm -r lint`: zero new errors compared to the base branch. Pre-existing baseline lint errors are acceptable as long as the diff doesn't add to them — diff against `origin/<base>` for the changed files if needed to confirm.
- `pnpm -r test:run`: all tests pass.
- `pnpm -r typecheck`: zero new errors in the touched files.
...
0. **Pre-flight gate is non-negotiable** (Phase 0). If `apps/web` or `apps/api` code changed, the check suite must pass before pushing. A broken build doesn't deserve a reviewer's time. The only exception is docs/asset-only PRs.
