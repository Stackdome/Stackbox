...
| `apps/web/src/components/branded/brand-icon-registry.ts` | `frontend/src/components/branded/brand-icon-registry.ts` | repaired: trimmed to docker, github, gitlab, bitbucket, gitea (plus light variants); history preamble deleted | none (covered by `ProviderLogo`) |
...
| `apps/web/src/components/branded/provider-logo.tsx` | `frontend/src/components/branded/provider-logo.tsx` | repaired: `providerId` takes a plain `string` instead of the dropped `ProviderId` from `lib/git-integrations`; history preamble deleted | `Branded/ProviderLogo` |
